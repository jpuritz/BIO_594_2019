...
Input the genotype into R. First open R by typing 'R' in the command line:
...
Each row contains the genotypes of an individual.
Each column contains the genotypes of a site (?).
...
```{r}
 ...
 plot(k2,pch=16,cex=3,col=1:5+1,ylab="distance 2. dimension",xlab="distance 1.  dimension",main="Multiple dimension scaling (MDS)")
 points(k2,pch=as.character(1:5))
 
```

* Can you find an example of where the 2 dimensional representation does not capture the true pairwise distances?

The distance 1. dimension captures the true pairwise distance pretty well. Confused about the distance 2nd dimension.
...
To remove different patterns of variation over the individuals. There is also the possibility of population stratification and batch effects, which normalizing seems to take care of. 
...
I does not appear like it worked that well. 
...
```
 ...
 The eigenvector values for the covariance matrix and normalized genotypes are the same, but the sign is flipped. 
 ...
 Everything is flipped and axis scales are different. 
